...
希尔排序
1.希尔排序是插入排序的升级版本
举例我们有数字 8 16 1 4 5 2 9 5 6

我们把这些数字分成三组 8 4 9, 16 5 5, 1 2 6

把这三组分别利用插入法排序 得到序列
4 8 9， 5 5 16, 1 2 6

然后整个的数列就变为了 4 5 1 8 5 2 9 16 6
...
整个数组就变成了 1 2 4 5 5 6 8 9 16



编程思想，首先先断定我们把这个数组分成几组
...
void Shell_Sort(uint32* buff, uint32 len)
{
	...
	do {
		gap = gap / 3 + 1;            //分组 我们把被测数组分成3组
		for (uint32 i = gap; i < len; i++)
		{
			if (buff[i - gap] > buff[i])          //在这里面我们把新分得数组进行排序
			{
				temp = buff[i];                   //把这个小的数据记录下来
				//for(j = ; )
